...
for train_idx, test_idx in cv.split(labels):
    ...
    mdm     = [MDM(metric=dict(mean='riemann', distance='riemann')) for _ in range(fea_N)]

    Filter  = [Covariances() for _ in range(fea_N)]


    ss      = preprocessing.StandardScaler()

    X_filter_train = np.array(tuple(Filter[x].fit_transform(feature_data[x,train_idx,:,:]) for x  in range(fea_N)))
    X_filter_test  = np.array(tuple(Filter[x].transform(feature_data[x,test_idx,:,:]) for x  in range(fea_N)))


    X_train = ss.fit_transform(np.concatenate(tuple(mdm[x].fit_transform(X_filter_train[x,:,:,:],label_train) for x  in range(fea_N)),axis=-1))
    X_test  = ss.transform(np.concatenate(tuple(mdm[x].transform(X_filter_test[x,:,:,:]) for x  in range(fea_N)),axis=-1))
    ...
    nn.fit(X_train, y_train, batch_size = 32, epochs = 400)
